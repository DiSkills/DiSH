...
static void wordlist_callback_get_length(struct wordlist_item *item,
        void *userdata)
{
    int *plen = userdata;
    (*plen)++;
...
int wordlist_get_length(const struct wordlist_t *wordlist)
{
    int count = 0;
    wordlist_traverse(wordlist, wordlist_callback_get_length, &count);
    return count;
}
